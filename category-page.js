...
(() => {
  ...
  if (params.has('category') && main) {
    ...
    if (template) {
      ...
      document.title = `${template.dataset.categoryTitle} | Benchvale Scientific`;
      document.querySelector('meta[name="description"]')?.setAttribute('content', template.dataset.categoryDescription);
      document.querySelector('link[rel="canonical"]')?.setAttribute('href', `https://benchvalescientific.com/products.html?category=${encodeURIComponent(params.get('category'))}`);
    } else {
      ...
      document.title = 'Category not found | Benchvale Scientific';
    }
  }
